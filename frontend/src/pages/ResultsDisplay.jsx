...
const ResultsDisplay = ({ data }) => {
    ...
    // ---- Helper: Render AI explanation with proper structure ----
    const renderAIExplanation = (text) => {
        if (!text) return '<p>No explanation available.</p>';
        
        const html = text
            // 1. Remove stray markdown artifacts
            .replace(/^\s*#+\s*$/gm, '')
            .replace(/^\s*---+\s*$/gm, '')
            .replace(/\n{3,}/g, '\n\n')
            .trim()
            ...
            // 2. Section headings
            .replace(/^##\s+(.+)$/gm, '<h4 class="ai-section">$1</h4>')
            .replace(/^###\s+(.+)$/gm, '<h5 class="ai-subsection">$1</h5>')
            ...
            .replace(
                /(\d+\.?\d*)\s*(g\/dL|mg\/dL|mmol\/L|%|cells\/cumm|Lakh\/cumm|million\/cumm|fL|pg|U\/L|mL\/min)/gi,
                '<span class="ai-value">$1 $2</span>'
            )
            
            // 5. Bullet lines
            .replace(/^[-•]\s+(.+)$/gm, '<li class="ai-list-item">$1</li>')
            
            // 6. Action lines (→ What to do: ...)
            .replace(/^→\s*(.+)$/gm, '<div class="ai-action">→ $1</div>')
            
            // 7. Wrap consecutive <li> in <ul>
            .replace(
                /(<li class="ai-list-item">.+?<\/li>\n?)+/g,
            ...
            <div className="results-header-modern">
                <div className="header-left">
                    ...
                    <div className="header-meta">
                        ...
                        {processedData.document_type && processedData.document_type !== 'Other' && (
                            <span className="badge-report-type">
                                📄 {processedData.document_type} Report
                            </span>
                        )}
            ...
            <div className="two-column-grid-large">
                ...
                {data.ai_explanation && (
                    <div className={`ai-explanation-large ${data.ai_explanation.success ? '' : 'fallback'}`}>
                        ...
                        <div
                            className="ai-content-large"
                            dangerouslySetInnerHTML={{
                                __html: renderAIExplanation(data.ai_explanation.explanation),
                            }}
                        />
